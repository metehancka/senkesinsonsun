...
class Bag:
    ...
    def __init__(self):
        ...
        self.data = []
        path = "C:\\Repositories\\opensource\\face\\face_recognition\\"
        print('ilk video yolu ' + path)
        ...
        while True:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                print('Ilk video, Yuz daha once kaydedildi mi: ' + str(result))
                if(result is False):
                    print('yuz klasorde yok, kaydediliyor')
        ...
        # read all video frame by frame
        print('ikinci while dongusu')
        while True:
            #break
            # Grab a single frame of video
            print('frame nuber: ' + str(frame_number))
            if (frame_number < 2900):
                print('frame number 2900 den kucuk, frame islenmeyecek ' + str(frame_number))                
                frame_number += 1
                continue
            if (frame_number > 30000):
                print('frame number 3000 den buyuk donguden ckiliyor')
                input_movie.release()
                cv2.destroyAllWindows()
                break
            ...
            if ret == False:
                print('ikinci video ret geldi while dongusunden cikiliyor')
                input_movie.release()
                cv2.destroyAllWindows()
                break

            # Loop through each face in this frame of video
            # print('ikinci for dongusu')
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                print('Ikinci video, yuz var mi: ' + str(result))
                if(result is False):
                    print('yuz klasorde yok, kaydediliyor')
        ...
        print('ilk videoda bulunan yuzler for dongusune sokuluyor')
        ...
        for file in os.listdir(directory):
            ...
            print('Ilk klasorden alinan yuzun ikinci klasordeki ile karsilastirma sonucu: '+ str(result))
            if(result is False):
                print('not found klasorune kaydediliyor')
                cv2.imwrite('not_found_faces\\'+ str(uuid.uuid1()) + '.jpg', firstImage)
            else:
                print('found klasorune kaydedliyoru')
